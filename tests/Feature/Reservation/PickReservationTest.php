...
test('warehouse operator can fully pick reservation without quantity param', function () {
    ...
    expect($history)->not->toBeNull()
        ...
        ->and($history->to_status)->toBe(ReservationStatus::Picked)
        ->and($history->quantity_affected)->toBe(5)
        ->and($history->actor_id)->toBe($this->warehouseOperator->id);
});

test('warehouse operator can partially pick reservation', function () {
    $response = $this->actingAs($this->warehouseOperator)
        ->postJson("/api/v1/reservations/{$this->reservation->id}/pick", ['quantity' => 2], $this->headers);
    ...
    expect($this->reservation->status)->toBe(ReservationStatus::Open)
        ->and($this->reservation->quantity_picked)->toBe(2);

    $this->inventory->refresh();
    expect($this->inventory->quantity_reserved)->toBe(3)
        ->and($this->inventory->quantity_picked)->toBe(2);
});

test('subsequent pick consumes remaining reserved quantity', function () {
    $this->actingAs($this->warehouseOperator)
        ->postJson("/api/v1/reservations/{$this->reservation->id}/pick", ['quantity' => 2], $this->headers)
        ->assertStatus(200);

    $response = $this->actingAs($this->warehouseOperator)
        ->postJson("/api/v1/reservations/{$this->reservation->id}/pick", ['quantity' => 3], $this->headers);
...
test('picking more than remaining reserved quantity returns 422', function () {
    $response = $this->actingAs($this->warehouseOperator)
        ->postJson("/api/v1/reservations/{$this->reservation->id}/pick", ['quantity' => 6], $this->headers);
    ...
    expect($this->inventory->quantity_reserved)->toBe(5)
        ->and($this->inventory->quantity_picked)->toBe(0);
});

test('picking released reservation returns 409', function () {
...
test('non-warehouse operator cannot pick reservation', function () {
